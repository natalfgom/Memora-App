...
import com.example.memora_app.modelos.PreguntasOri
import com.google.firebase.firestore.FirebaseFirestore
import java.time.LocalDate

class JuegoOrientacionActivity : AppCompatActivity() {

    private lateinit var preguntas: List<Pregunta>
    private var indice = 0
    private var aciertos = 0
    private lateinit var pacienteId: String
    private val categoria = "Orientación"
    private lateinit var botonSiguiente: Button

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.juego_comprension) // Reutiliza mismo layout
        ...
        if (dificultadRecibida != null) {
            Log.d("JuegoOrientacion", "Dificultad recibida por intent: '$dificultadRecibida'")
            preguntas = PreguntasOri.getPreguntasPorDificultadOri(dificultadRecibida)
            Log.d("JuegoOrientacion", "Preguntas cargadas: ${preguntas.size}")
    ...
    private fun guardarResultado() {
        ...
        val intent = Intent(this, ResultadosCalculoActivity::class.java) // Puedes crear ResultadosOrientacionActivity si quieres personalizarlo
