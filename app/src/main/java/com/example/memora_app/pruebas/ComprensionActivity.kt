...
class ComprensionActivity : AppCompatActivity() {
    ...
    private fun guardarResultadosEnFirebase() {
        ...
        db.collection("Pacientes").document(pacienteID)
            .collection("Pruebas").document(fechaActual) // Guarda los resultados con la fecha actual
    ...
    private fun clasificarDificultad(puntaje: Int): String {
        return when {
            puntaje >= 4 -> "Dificultad Alta (Normal)"  // 5 o 4
            puntaje in 2..3 -> "Dificultad Media (Leve Deterioro)"  // 2 o 3
            else -> "Dificultad Baja (Deterioro Severo)"  // 1 o 0
        }
    }
}
